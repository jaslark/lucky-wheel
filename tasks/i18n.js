...
gulp.task("i18n", done => {
  ...
  files.forEach(function(path) {
    ...
    $("[translate], [translate-placeholder], [translate]").each(function(
      i,
      el
    ) {
      let $el = $(el),
  ...
  fs.writeFileSync("./translations.json", JSON.stringify(fromHtml), "utf8");
  done();
});
...
function extractTranslationsFromTsFiles() {
  ...
  files.forEach(path => {
    ...
    lines = { ...lines, ...getAllMatches(/message =\s*'(.+?)',?/g, content) };

    // extract "toast" strings
    lines = {
      ...lines,
      ...getAllMatches(/this.translate.instant\('(.+?)'\)/g, content)
    };
